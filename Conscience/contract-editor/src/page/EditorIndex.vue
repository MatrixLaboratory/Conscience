                        ...
                        <el-col :span="4">
                          <el-dropdown @command="handleCommand" trigger="click">
                            ...
                            <el-dropdown-menu slot="dropdown">
                              ...
                              >{{menuLang.fileFunc.download}}</el-dropdown-item>
                              <el-dropdown-item
                                icon="el-icon-edit"
                                :command="file + '-edit'"
                              >{{menuLang.fileFunc.edit}}</el-dropdown-item>
                              <el-dropdown-item
                                icon="el-icon-delete"
                                :command="file + '-delete'"
                              >{{menuLang.fileFunc.delete}}</el-dropdown-item>
                            </el-dropdown-menu>
                          </el-dropdown>
                        </el-col>
                      </el-row>
                  ...
                  <el-footer :height="footerH">
                    <el-container>
                      ...
                      >
                        ...
                        <el-card
                          class="box-card"
                          v-for="(logger, index) in compileLoggers"
                          :key="index"
                        >
                          <div
                            slot="header"
                          ...
                          <div class="text item" style="color: white;">
                            ...
                            <xmp v-show="logger.returns !== '' && logger.returns !== undefined">
                              <code>method-return: {{logger.returns}}</code>
                            </xmp>
                          </div>
                          <div
                          ...
                          >
                            ...
                            <span>
                              Submit your IOST dapp to
                              <a
                                href="https://dapp.review/"
                              >https://dapp.review/</a> to track all trasactions today!
                            </span>
                          </div>
                        </el-card>
                        <!-- log card -->
            ...
            <el-main slot="secondPane" class="setting-window" v-show="settingSelect.setting.show">
              <el-row :gutter="20" class="setting-about-card-row">
                <el-form
                  id="setting"
                  ref="settingForm"
                  :model="settingSelect.setting.data"
                  label-width="100px"
                  label-position="left"
                >
              ...
              <el-carousel-item v-for="(item, index) in menuLang.resource.data" :key="index">
                <div class="photo">
                  ...
                  <div class="photo-overlay">
                    <h2>{{item.title}}</h2>
                    <el-button type="primary" @click="openTab(item.url)">{{item.button}}</el-button>
                  </div>
                </div>
              </el-carousel-item>
            </el-carousel>
...
export default {
  ...
  methods: {
    ...
    runResult(runResult) {
      ...
      if (runResult.status === "failed") {
        this.compileLoggers.push({
          ...
          description:
            runResult.trx + ": method run failed! \n" + runResult.message,
          style: "error"
        });
      }
    },
    ...
    saveSettings() {
      if (
        this.saveSetting.langMode != "" &&
        this.saveSetting.langMode !== null
      ) {
        this.langMode = this.saveSetting.langMode;
      }
      if (
        this.saveSetting.themeMode != "" &&
        this.saveSetting.themeMode !== null
      ) {
      ...
      if (
        this.saveSetting.codeThemeMode !== "" &&
        this.saveSetting.codeThemeMode !== null
      ) {
